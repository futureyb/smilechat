        ...
        >
          ...
          >
            <div
              class="chat-text"
              v-if="item.type == 0"
              @contextmenu.prevent="rightClickMsg($event,item.message)"
              v-html="item.message"
            ></div>
...
const rightClickMsg = (e,message) => {
  ...
  let aa = loading(ac, e.pageX, e.pageY);
...
const scrollBottom = (behavior = "auto") => {
  nextTick(() => {
    ...
    let target = setChatRef.value.filter(
      (item) => item.getAttribute("data-id") == setChatRef.value.length - 1
    );
